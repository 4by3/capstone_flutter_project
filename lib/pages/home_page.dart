        ...
        if (mode == 'easy' && features.every((f) => f['score'] == 5)) {
          ...
          if (mounted) {
            Future.microtask(() async {
              ...
              // Fix: Properly reset all features for hard mode and force UI refresh
              if (mounted) {
                // Reset all features to 0 score and not started for hard mode
                for (var feature in features) {
                  feature['score'] = 0;
                  feature['started'] = 0;
                }

                // Clear all hard mode question progression data
                final prefs = await SharedPreferences.getInstance();
                for (var feature in features) {
                  String featureName = feature['name'];
                  // Clear hard mode question index and other quiz state
                  await prefs
                      .remove('${featureName}_hard_currentQuestionIndex');
                  await prefs.remove('${featureName}_hard_answeredQuestions');
                  await prefs.remove('${featureName}_hard_correctAnswers');
                  await prefs.remove('${featureName}_hard_totalQuestions');
                  await prefs.remove('${featureName}_hard_quizCompleted');
                  // Clear any other hard mode quiz state keys that might exist
                  await prefs.remove('${featureName}_hard_questionHistory');
                  await prefs.remove('${featureName}_hard_userAnswers');
                }
                ...
                setState(() {
                  ...
                  _isCardHovered = List.generate(features.length, (_) => false);
                  _isVideoHovered =
                      List.generate(features.length, (_) => false);
                });

                // Ensure scores are saved with hard mode prefix
                        ...
                        AnimatedPositioned(
                          ...
                          child: Container(
                            ...
                            child: Icon(
                              ...
                              size: 12,
                              color:
                                  isDarkMode ? primaryBlue : Colors.orange[700],
                            ),
                          ),
                        ),
                      ],
